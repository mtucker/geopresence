...
public class GeocellManager {
    ...
    public static final <T extends LocationCapable> List<T> proximityFetch(Point center, int maxResults, double maxDistance, Class<T> entityClass, GeocellQuery baseQuery, PersistenceManager pm, int maxGeocellResolution) {
        ...
        while(!curGeocells.isEmpty()) {
            ...
            if(baseQuery != null && baseQuery.getBaseQuery() != null){
            	
            	baseQueryString = baseQuery.getBaseQuery() + " && ";
            	
            }
            
            Query query = pm.newQuery(entityClass, baseQueryString + "geocells.contains(geocellsParameter)");

            if(baseQuery == null || baseQuery.getDeclaredParameters() == null || baseQuery.getDeclaredParameters().trim().length() == 0) {
                query.declareParameters("java.util.List geocellsParameter");
            } else {
                query.declareParameters(baseQuery.getDeclaredParameters() + ", java.util.List geocellsParameter");
            }

            List<T> newResultEntities;
            if(baseQuery == null || baseQuery.getParameters() == null || baseQuery.getParameters().isEmpty()) {
