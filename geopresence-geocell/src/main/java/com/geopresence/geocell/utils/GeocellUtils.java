...
public final class GeocellUtils {
	...
	private static final Logger logger = LoggerFactory.getLogger(GeocellUtils.class);
    ...
    private static final int MAX_FEASIBLE_BBOX_SEARCH_CELLS = 800;
    ...
    public static List<Tuple<int[],Double>> distanceSortedEdges(List<String> cells, Point point) {
        ...
        for(String cell : cells) {
        	...
        	BoundingBox box = computeBox(cell);
    ...
    public static List<String> getCellsWithinRadius(Point p, Double distance){
    	
    	List<String> cellsWithRadius = new ArrayList<String>();
    	
    	BoundingBox bbox = boundingCoordinates(p, distance);
    	
    	int resolution = 8;
    	
    	String cellNE = GeocellUtils.compute(bbox.getNorthEast(), resolution);
        String cellSW = GeocellUtils.compute(bbox.getSouthWest(), resolution);
        
        List<String> cells = GeocellUtils.interpolate(cellNE, cellSW);
        
        for(String cell : cells){
        	
        	if(GeocellUtils.pointDistance(cell, p) <= distance){
        		
        		cellsWithRadius.add(cell);
        		
        	}
        	
        }
    	
    	return cellsWithRadius;
    	
    }
	
	public static boolean withinRadius(Point p, String cell, Double distance){
		
		return  GeocellUtils.containsPoint(cell, p)
		|| GeocellUtils.pointDistance(cell, p) <= distance;
		
	}
	
	public static BoundingBox boundingCoordinates(Point p, double distance) {

		// angular distance in radians on a great circle
		double radDist = distance / RADIUS;
		
		double radLat = Math.toRadians(p.getLat());
		double radLon = Math.toRadians(p.getLon());
		...
		return new BoundingBox(maxLatDeg, maxLonDeg, minLatDeg, minLonDeg);
	}
    ...
    public static List<String> generateGeoCell(Point point) {
        ...
        String geocellMax = GeocellUtils.compute(point, GeocellUtils.MAX_GEOCELL_RESOLUTION);
        for(int i = 1; i < GeocellUtils.MAX_GEOCELL_RESOLUTION; i++) {
    ...
    public static List<String> bestBboxSearchCells(BoundingBox bbox, CostFunction costFunction) {
    	...
    	// Compute the geocell at max resolution at the NE and SW corners of the bounding box
        String cellNE = GeocellUtils.compute(bbox.getNorthEast(), GeocellUtils.MAX_GEOCELL_RESOLUTION);
        String cellSW = GeocellUtils.compute(bbox.getSouthWest(), GeocellUtils.MAX_GEOCELL_RESOLUTION);
        ...
        for(int curResolution = minResolution; curResolution < GeocellUtils.MAX_GEOCELL_RESOLUTION + 1; curResolution++) {
        ...
        if(logger.isDebugEnabled()){
        	logger.debug("Calculate cells "+ StringUtils.join(minCostCellSet, ", ")+" in box ("+bbox.getSouth()+","+bbox.getWest()+") ("+bbox.getNorth()+","+bbox.getEast()+")");
        }
